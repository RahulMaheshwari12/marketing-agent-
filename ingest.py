...
#Core auto-ingestion pipeline function that orchestrates the entire process of loading, extracting, and uploading data.
async def auto_ingest_event(event_id: str, file_path: str, category: str):
    """Loads file, runs AI to extract metadata for Firebase, and uploads chunks to Qdrant."""
    valid_categories = ["permanent", "professional", "campaign"]
    if category not in valid_categories:
        raise ValueError(f"Invalid category '{category}'. Must be one of {valid_categories}")
    print(f"Starting auto-ingestion for event '{event_id}' [{category}] from '{file_path}'...")
    #Loading the file
    raw_docs = load_documents_from_file(file_path)
    
    # Combine page texts to send to the extraction LLM
    full_text = "\n".join([doc.page_content for doc in raw_docs])
    
    #Extracting metadata using Gemini AI
    extracted_metadata = await extract_event_metadata(full_text)
    print(f"AI Extracted: {extracted_metadata}")
    
    #Saving extracted metadata to Firebase
    await ingest_static_metadata(event_id, extracted_metadata)
    #Spliting text document into chunks for vector search (configured with list separators)
    ...
    # Step F: Upload vectors to Qdrant
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Auto-ingest event brochure into Qdrant and Firebase.")
    parser.add_argument("event_id", type=str, help="Unique identifier for the event.")
    parser.add_argument("file_path", type=str, help="Path to the event brochure file (.txt or .pdf).")
    parser.add_argument("category", type=str, choices=["permanent", "professional", "campaign"], help="Category of the event.")

    #Parsing command-line arguments
    args = parser.parse_args()


#wrapping event execution in an async function to handle exceptions and provide clear error messages
    async def main():
        try:
            await auto_ingest_event(args.event_id, args.file_path, args.category)
